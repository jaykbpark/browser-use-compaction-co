...
class ArizeEvalTracer:
    ...
    def step_span(
        ...
        *,
        ...
        with self.tracer.start_as_current_span("browserdelta.eval.step") as span:
            ...
            span.set_attribute("browserdelta.visual_raw_changed_pct", observation.visual_raw_changed_pct)
            if observation.visual_ssim_score is not None:
                span.set_attribute("browserdelta.visual_ssim_score", observation.visual_ssim_score)
            if observation.visual_phash_distance is not None:
                span.set_attribute(
                    "browserdelta.visual_phash_distance", observation.visual_phash_distance
                )
            ...
            span.set_attribute("browserdelta.expected_action.target", expected_next_action.target or "")
            span.set_attribute("input.value", _json_dumps({
    ...
    def record_step(self, span: Any | None, result: ReplayStepResult) -> None:
        ...
        span.set_attribute(
            "browserdelta.predicted_action.target", result.predicted_next_action.target or ""
        )
        span.set_attribute("output.value", _json_dumps({
